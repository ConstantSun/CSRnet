...
class CSRNet_student(nn.Module):
    ...
    def __init__(self, cpr: float, convert_weight = '0'):
        ...
        print("init CSRNet_student _____________")
...
def FSP_matrix(f1: torch.tensor, f2: torch.tensor)  : # (f size: b, c, h, w)
    ...
    return torch.bmm(f1.view(_b1,_c1,_h1*_w1), f2.view(_b2,_c2,_h2*_w2).transpose(1,2))/(_h1*_w1)



def is_unique(a: torch.Tensor):  
    t = a[0][0]
    _ele = t[0][0]
    for i in range(t.shape[0]): 
        for j in range(t.shape[1]):
            if t[i][j] - _ele > 0.0001: 
                return False
    return True
...
def triplet_loss(s_kd_list: list, t_kd_list: list, s_resize_list: list, t_resize_list: list, s_out, t_out, target) :
    ...
    """
    s_kd_list: student knowledge distillation list.  - for L_intra - pattern transfer
    t_kd_list: teacher knowledge distillation list.  - for L_intra - pattern transfer
    s_resize_list: student knowledge distillation after resize - for L_inter - relation transfer
    t_resize_list: teacher knowledge distillation after resize - for L_inter - relation transfer
    ...
    mse = nn.MSELoss(size_average=False).cuda() 
    ...
    for i in range(_num_elements): #  Here we have 6 extract layers of shape (b, c, h, w)
        ...
        print('L_intra:', 1.*bchw[0]*bchw[2]*bchw[3] - cos(s_kd_list[i],t_kd_list[i]).sum(), 
        '\nbatch*height*width = ',1.*bchw[0]*bchw[2]*bchw[3] ,
          '\ncos = ', cos(s_kd_list[i],t_kd_list[i]).sum()) 

    L_inter = 0. 
    for i in range(_num_elements):  #  = 6
        for j in range(_num_elements):
            if j > i :
                ...
                # print("Norm FSP_matrix: ", torch.norm( FSP_matrix(t_resize_list[i], t_resize_list[j]) - FSP_matrix(s_resize_list[i], s_resize_list[j]) ))
                L_inter += mse(FSP_matrix(t_resize_list[i], t_resize_list[j]), FSP_matrix(s_resize_list[i], s_resize_list[j]))
                print(mse(FSP_matrix(t_resize_list[i], t_resize_list[j]), FSP_matrix(s_resize_list[i], s_resize_list[j])))
    ...
    _h_min = s_out.shape[2] if s_out.shape[2] < target.shape[2] else target.shape[2]
    _w_min = s_out.shape[3] if s_out.shape[3] < target.shape[3] else target.shape[3]


    # print("2)____________\n")
    # print("_h_min: ", _h_min)
    # print("_w_min: ", _w_min)
    # print(s_out[0,0,:_h_min,:_w_min], "\n", target[0,0,:_h_min,:_w_min])


    # L_m = torch.norm(t_out - s_out) + torch.norm(s_out[0,0,:_h_min,:_w_min] - target[0,0,:_h_min,:_w_min])
    L_m = mse(t_out, s_out) + mse(s_out[0,0,:_h_min,:_w_min], target[0,0,:_h_min,:_w_min] )
    ...
    print("STUDENT HEATMAP : \n", s_out)
    print("IS UNIQUE: ", is_unique(s_out))

    # torch.save(s_out, 's_out_1.pt')
